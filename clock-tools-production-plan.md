...
| C5 | **Partial.** Keeper anchors count against the shared `MCP_LOG_BUDGET`; per-owner trigger cap 100; minimum interval 60 s; 30-day horizon. Per-`sub` billing still deferred | `keeper-runtime.ts` `budgeted()` |
| C6 | **Deferred by design.** Webhook delivery is **off** on the hosted deployment (no `KEEPER_WEBHOOK_SECRET`); `webhook_url` is refused with a clear message and poll (`timer_status`) is the delivery path. Enable only with the secret + `KEEPER_WEBHOOK_ALLOWLIST` + DNS-pinned SSRF | `timer-tools.test.mjs` covers refused/accepted |
