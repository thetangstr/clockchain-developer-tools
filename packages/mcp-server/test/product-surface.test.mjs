...
// full surface (default / MCP_SURFACE=full) still exposes all 50 tools.
//
...
test('surface="full" (explicit) registers all 50 tools including get_time (offline fake-server)', () => {
  ...
  assert.ok(names.includes("agent_handshake_submit"), "full surface must include generic handshake submit");
  assert.ok(names.includes("agent_handshake_invite"), "full surface must include generic handshake invitation");
  assert.equal(names.length, 50, `Expected 50 tools, got ${names.length}: ${JSON.stringify(names)}`);
});

test("surface omitted (default) is identical to full — 50 tools (offline fake-server)", () => {
  const names = collectToolNames({});
  assert.equal(names.length, 50, `Expected 50 tools (default), got ${names.length}`);
...
test("MCP_SURFACE unset (default full) → tools/list has 50 tools (HTTP server)", async () => {
  ...
  try {
    ...
    const names = (body.result.tools ?? []).map((t) => t.name);
    assert.ok(names.includes("get_time"), "full surface must include get_time");
    assert.equal(names.length, 50, `Expected 50 tools on full surface, got ${names.length}`);
  } finally {
