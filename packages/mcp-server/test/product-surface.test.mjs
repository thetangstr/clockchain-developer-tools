...
// full surface (default / MCP_SURFACE=full) still exposes all 45 tools.
//
...
test('surface="full" (explicit) registers all 45 tools including get_time (offline fake-server)', () => {
  ...
  assert.equal(names.length, 45, `Expected 45 tools, got ${names.length}: ${JSON.stringify(names)}`);
});

test("surface omitted (default) is identical to full — 45 tools (offline fake-server)", () => {
  const names = collectToolNames({});
  assert.equal(names.length, 45, `Expected 45 tools (default), got ${names.length}`);
...
test("MCP_SURFACE unset (default full) → tools/list has 45 tools (HTTP server)", async () => {
  ...
  try {
    ...
    const names = (body.result.tools ?? []).map((t) => t.name);
    assert.ok(names.includes("get_time"), "full surface must include get_time");
    assert.equal(names.length, 45, `Expected 45 tools on full surface, got ${names.length}`);
  } finally {
