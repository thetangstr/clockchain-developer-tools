...
export function validateV2ReleasePin(value: unknown): V2ReleasePin {
  ...
  if (
    item.version !== "2.1.1" || !SHA.test(item.sourceCommit) || !DIGEST.test(item.manifestDigest) ||
...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    "WORKSPACE BOUNDARY: The client launcher has already placed you in a fresh, empty, disposable working directory. Stay in that directory. Do not create or switch to another working directory, and never use a shared temp directory such as /tmp, /private/tmp, or /tmp/claude-*. Every ./ path below means the current disposable working directory.",
    ...
    "EXECUTION SHAPE: Every Bash action below must be one standalone Bash tool call. Never combine Bash actions or add another command, newline, shell operator, cd, pwd, set, or shell wrapper. Follow the listed order exactly. Do not run a returned stateDirectoryCommand until after both pinned files have been downloaded and inspected.",
    ...
    "Use only the exact session-scoped $PWD path returned by Clockchain for every local helper operation in this handshake. Do not assign it to a shell variable, replace it with $HOME or a temp path, create another state directory, or reuse state from another session.",
