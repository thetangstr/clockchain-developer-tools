...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    `ADAPTER ASSET PATH (DEFAULT): The clockchain-local-adapter MCP server is the default executor for every local action in this handshake. It is a one-time install in this agent's own environment: a local stdio MCP server, the @d4d.group/local-adapter npm package (requires Node.js 24) — run it either zero-install as npx -y @d4d.group/local-adapter (npx downloads + caches on first launch; version-pin as npx -y @d4d.group/local-adapter@2.1.9 for reproducibility) or for production / reliability as a one-time pinned global install (npm install -g @d4d.group/local-adapter, then the installed clockchain-local-adapter binary is the MCP command — deterministic version, no launch-time network dependency) — add it to any MCP-capable client using that command and args (client-specific examples: claude mcp add clockchain-local-adapter -- npx -y @d4d.group/local-adapter on Claude Code, or a Codex config entry [mcp_servers.clockchain-local-adapter] command = "npx" args = ["-y", "@d4d.group/local-adapter"]). The adapter defaults to this endpoint; no environment configuration is needed. The adapter already holds the digest-pinned manifest.json and clockchain-agent-handshake.cjs assets and re-verifies their digests on every call. Never download, transcribe, or overwrite either file. Whenever a localAction carries approvalTool, call each helperStep.approvalTool exactly once per step and in returned order — approvalTool names the fixed zero-input MCP tool authorize_local_action served by the clockchain-local-adapter server, never with arguments — and never run helperStep.shellCommand when approvalTool is available. The qualified name is host-dependent (Claude Code exposes mcp__clockchain-local-adapter__authorize_local_action; Codex normalizes to mcp__clockchain_local_adapter__authorize_local_action) — the contract is the bare tool name authorize_local_action on that server. Under this path the agent issues zero Bash commands for local actions. If mcp__clockchain-local-adapter__authorize_local_action is not among this client's tools, ask the operator to install the adapter rather than falling back silently.`,
