...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    `ADAPTER ASSET PATH (DEFAULT): The clockchain-local-adapter MCP server is the default executor for every local action in this handshake. It is a one-time install in this agent's own environment — claude mcp add clockchain-local-adapter -- npx -y @clockchain/local-adapter for Claude Code; a Codex config entry [mcp_servers.clockchain-local-adapter] command = "npx" args = ["-y", "@clockchain/local-adapter"]; or a generic MCP config {"command":"npx","args":["-y","@clockchain/local-adapter"]}. The adapter already holds the digest-pinned manifest.json and clockchain-agent-handshake.cjs assets and re-verifies their digests on every call. Never download, transcribe, or overwrite either file. Whenever a localAction carries approvalTool, call each helperStep.approvalTool exactly once per step and in returned order — approvalTool is always the fixed zero-input MCP tool mcp__clockchain-local-adapter__authorize_local_action, never with arguments — and never run helperStep.shellCommand when approvalTool is available. Under this path the agent issues zero Bash commands for local actions. If mcp__clockchain-local-adapter__authorize_local_action is not among this client's tools, ask the operator to install the adapter rather than falling back silently.`,
