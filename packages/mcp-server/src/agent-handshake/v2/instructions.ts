...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    "After agent_handshake_invite, use roleAccess (the same value retained as initiatorAccess for compatibility) only for Initiator role-scoped tools; responderInvitation is the single-use value to copy to the Responder. Never substitute responderInvitation for roleAccess.",
    ...
    "After invitation acceptance, use roleAccess (the same value retained as responderAccess for compatibility) only for Responder role-scoped tools. The original invitation is consumed; never use it as an access argument.",
