...
export function buildV2Instructions(rawPin: unknown): string {
  ...
  return [
    ...
    "EXECUTION SHAPE: Every Bash action below must be one standalone Bash tool call. Never combine Bash actions or add another command, newline, shell operator, cd, pwd, set, or shell wrapper. Follow the listed order exactly. Do not create ./clockchain-state until after both pinned files have been downloaded and inspected.",
    ...
    "When a localAction includes helperSteps or helperStep, append each argvAfterVerifiedPrefix array to the verified prefix in the exact order returned. Replace only REPLACE_WITH_EXACT_ABSOLUTE_STATE_DIR with the one absolute state directory you created. Never reconstruct, edit, or re-encode a returned payload. If an operation's argvAfterVerifiedPrefix does not include --payload-base64url, do not add that flag or any payload.",
