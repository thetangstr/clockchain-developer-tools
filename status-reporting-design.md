...
| Per-tool/stage counts + latencies | `clockchain_handshake_tool_{calls_total,duration_seconds}` by `{tool,result}`; `clockchain_handshake_stage_total{stage}`; `clockchain_handshake_failures_total{reason}`; `clockchain_handshake_completed_total{outcome}`; `clockchain_handshake_inflight` gauge |
...
| Protocol/business health | `lastVerifiedHandshake` in status | Did a real handshake recently complete end-to-end (evidence only)? |
...
| `anchoring_gateway` | `ClockchainClient.getPoolHealth()` reachability |
| `pool_participation` | same call — `degraded` when the pool reports degraded (missing participation ⇒ call throws ⇒ down, never fabricated healthy) |
...
| `lastVerifiedHandshake` | `GET {RELAY}/v1/sessions/{current}/result` → `outcome=="VERIFIED"` + `issuedAtMs` |
...
| Handshake completions halt | business | `increase(clockchain_handshake_completed_total[1h]) == 0` during expected canary windows |
...
- **`/metrics` is private**: timing-safe Bearer compare; 404 when the token is
  unset — the route doesn't exist in unconfigured deployments.
