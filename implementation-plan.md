...
- **ERC-8004 registry default.** `resolve_agent` reads the **official ERC-8004
  Identity Registry** at `0x8004A818BFB912233c491871b3d84c89A494BD9e` on
  **Ethereum Sepolia** by default, so hosted and local identity reads do not
  require registry configuration. Override via `EVM_RPC_URL` / `ERC8004_CHAIN` /
  `ERC8004_REGISTRY_ADDRESS` for another deployment - no code change.
