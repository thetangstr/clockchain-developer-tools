...
Run and verify the Clockchain MCP server locally. The server exposes **50 tools
across six modules** (time, notarization, scheduler, audit, agent identity, TSA) over
stdio; this quickstart drives the verified core — **8 of the 50**, the time +
...
(should list `clockchain` with 50 tools), and ask: *"use clockchain to timestamp the
...
**50 tools total**. Cross-party verification is keyless: it reads the immutable on-chain
