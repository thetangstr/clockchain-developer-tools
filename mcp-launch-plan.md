...
- **ChatGPT app + broad push: gated** on truthful anchoring + per-user OAuth + per-user auth fix.
...
### Truthful anchoring — never report success on an un-anchored fire
...
**Cross-cutting:** both touch the same write handlers in `tools.ts` — do truthful anchoring first so per-user auth's per-user budget reuses that refactor.
